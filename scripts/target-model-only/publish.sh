...
if [[ ${MODEL_ONLY_SET_DEFAULT:-false} == true ]]; then
  printf '{"release_id":"%s"}' "$release_id" >"$temporary_dir/default.json"
  curl --fail-with-body --silent --show-error \
    --request POST "$AGENT_REGISTRY_URL/v1/agents/model-only/default-release" \
    --header "Authorization: Bearer $REGISTRY_PUBLISH_TOKEN" \
    --header "Content-Type: application/json" \
    --header "Idempotency-Key: model-only-default-${release_id}" \
