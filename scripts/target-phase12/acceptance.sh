...
for variable_name in KEYCLOAK_TOKEN_URL AGENT_REGISTRY_URL PORTAL_BFF_URL \
  RUNTIME_DATABASE_URL RUNNER_DATABASE_URL OIDC_CLIENT_ID OIDC_CLIENT_SECRET TEST_USER_CLIENT_ID \
...
if [[ ${PHASE12_PREFLIGHT_ONLY:-false} != true \
  ...
  PLANNING_ASSISTANT_VERSION=1.1.1 PLANNING_ASSISTANT_SET_DEFAULT=true \
...
uv run --isolated --with asyncpg==0.30.0 \
  ...
  --runner-database-url "$RUNNER_DATABASE_URL" \
