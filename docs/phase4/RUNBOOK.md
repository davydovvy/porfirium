...
Startup builds the independently locked time and MTG catalog MCP services, starts Agentgateway and the existing platform services, applies migration `0004_phase4_tool_audit`, verifies Bifrost's persisted deny-by-default MCP setting, and starts the portal API and both Temporal workflow versions. Agentgateway handles MCP traffic; Bifrost continues to handle model traffic.
...
Agent workflow IDs are `porfirium-agent-<turn-id>`. New Phase 4 runs use the `PorfiriumToolAgentWorkflowV2` definition; V1 remains registered for replay compatibility with existing histories. Search by workflow ID in Temporal UI. The turn's 32-character correlation ID is its W3C trace ID and Bifrost model-session ID; standard trace context is propagated through Agentgateway for MCP execution, so model generations, tool execution, and application observations appear under one Langfuse trace. Tool audit rows remain owner-scoped and are not exposed through a cross-user inspection endpoint.
