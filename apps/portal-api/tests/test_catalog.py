...
from portal_api.catalog import load_candidate, load_manifest, manifest_digest, validate_manifest
...
def test_filesystem_candidate_is_canonical_and_publishable() -> None:
    manifest, artifact, digest = load_candidate(CORRECTED_SEARCH_MANIFEST_PATH.parent)
    assert json.loads(artifact) == manifest
    assert digest == manifest_digest(manifest)


def test_filesystem_candidate_rejects_extra_file(tmp_path: Path) -> None:
    package = tmp_path / "tool-assistant/1.2.0"
    package.mkdir(parents=True)
    (package / "manifest.json").write_text(CORRECTED_SEARCH_MANIFEST_PATH.read_text())
    (package / "README.md").write_text("not part of the declarative artifact")
    with pytest.raises(ValueError, match="candidate_invalid:package_shape"):
        load_candidate(package)


def test_filesystem_candidate_rejects_duplicate_json_key(tmp_path: Path) -> None:
    package = tmp_path / "tool-assistant/1.2.0"
    package.mkdir(parents=True)
    source = CORRECTED_SEARCH_MANIFEST_PATH.read_text()
    duplicate = source.replace(
        '"schema_version": 2,', '"schema_version": 2, "schema_version": 2,'
    )
    (package / "manifest.json").write_text(duplicate)
    with pytest.raises(ValueError, match="candidate_invalid:duplicate_key"):
        load_candidate(package)
