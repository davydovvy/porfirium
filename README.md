...
Transition Increments 0–6 are complete and accepted; Increments 6.5 and 7 are implemented with final live acceptance pending. Agentgateway 1.4.0 is the sole model and MCP provider through `AgentgatewayModelGateway` and `AgentgatewayToolGateway`; Bifrost has been removed from runtime, configuration, and current verification paths. Every accepted Agent turn is pinned to an immutable version and self-contained run snapshot. Rerun the gates with `./scripts/migration-baseline/verify.sh`, `./scripts/agentgateway-spike/verify.sh`, and `./scripts/increment7/verify.sh`. Cumulative evidence is tracked in [Architecture Transition Results](docs/architecture/TRANSITION_RESULTS.md).

Increment 7 is implemented. The platform-owned `AgentRunWorkflow` executes immutable declarative run snapshots on the sole `porfirium-agent-runtime-v1` queue; the bundled `tool-assistant:1.1.0` release is the default generic release while `1.0.0` remains immutable historical data. See the [Increment 7 plan](docs/architecture/INCREMENT7_PLAN.md) and [implementation results](docs/architecture/INCREMENT7_RESULTS.md).
